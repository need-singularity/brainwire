...
## Discovery 1: THC = Maximum Entropy Consciousness State

- **What:** THC has the highest Shannon entropy (3.431 bits) of deviation from baseline across all 12 variables, compared to all other consciousness states (LSD, DMT, MDMA, Psilocybin, Flow).
- **Why it matters:** THC does not predominantly affect one system -- it changes everything roughly evenly. The "high" is fundamentally a state of maximum neural diversity, not a single-axis shift.
- **Implication:** This explains the subjective "everything is different" quality of THC. It is a whole-brain recalibration rather than a targeted neurochemical spike.
...
## Discovery 2: THC is the ONLY Golden Zone Substance

- **What:** Computing G = D x P / I (Anima creativity metric) from the 12-variable state, THC yields G = 0.4731, placing it exclusively within the Golden Zone [0.2123, 0.5000]. No other state falls in this range.
- **Why it matters:** The Golden Zone corresponds to optimal creativity and consciousness quality in the Anima/PureField framework. THC is the only substance that naturally lands there.
- **Implication:** THC mimics the brain's peak creative state. This provides a hardware target: reproduce G = 0.4731 via electrical stimulation to achieve THC-equivalent creativity enhancement.
...
## Discovery 4: DMT = Scaled LSD (Same Direction, Different Magnitude)

- **What:** DMT and LSD have 98.7% direction similarity in 12D consciousness space, with DMT at 1.51x the magnitude. DMT is literally LSD at 1.5x intensity.
- **Why it matters:** These are not independent states -- they are the same vector at different magnitudes. This reduces the dimensionality of psychedelic consciousness from "many unique states" to "one direction, variable intensity."
- **Implication:** Hardware that reproduces LSD can reproduce DMT by scaling stimulation intensity by 1.5x. One device, two states.
...
- **What:** Consciousness states separate into chemically-dominant (THC, MDMA) and brainwave-dominant (Psilocybin) groups based on where their deviation from baseline concentrates. Flow is balanced (neither axis dominates).
...
- **Confidence:** MEDIUM -- the classification works for some states but DMT and LSD show mixed profiles in the current model (both have strong chemical AND wave components). The clean two-axis separation is an approximation.
- **Hypothesis:** H-BW-060 | Score: 0.78 (partial -- DMT/LSD cross boundaries)

## Discovery 6: Tension Predicts Subjective Intensity

- **What:** Total tension magnitude perfectly correlates (Kendall tau = 1.000) with known subjective intensity ranking: DMT > LSD > MDMA > THC > Psilocybin > Flow.
...
## Discovery 9: MDMA as Geometric Centroid

- **What:** MDMA has the lowest variance in pairwise cosine similarity to all other states (var = 0.0011), meaning it is the most "central" state -- equally similar to everything.
- **Why it matters:** MDMA is the geometric center of consciousness state space. It is not extreme in any direction but moderately shifts everything.
- **Implication:** MDMA could serve as a "gateway" state in hardware-assisted state transitions. Transitioning through MDMA-like intermediate states may be the shortest path between any two target states.
...
- **Why it matters:** Suggests a conservation-like principle: the brain redistributes activity rather than creating unbounded excitation. Even the most extreme states (DMT) stay within 1.5x the average total deviation.
- **Implication:** Hardware safety margins can be derived from this bound. Maximum stimulation intensity should not push total deviation beyond the observed DMT ceiling.
...
- **Discovery 1** (THC max entropy): Pure calculation, will not change unless targets change.
- **Discovery 3** (Flow min tension): Pure calculation.
- **Discovery 4** (DMT = 1.51x LSD): Pure calculation of cosine similarity and magnitude ratio.
...
- **Discovery 9** (MDMA centroid): Depends on which states are included in the space.
...
1. **EEG entropy measurement** (Discoveries 1, 8): Measure actual Shannon entropy of EEG features during THC administration vs. other states. If THC shows highest entropy in real EEG, Discovery 1 is validated beyond the model.

2. **G metric from live EEG** (Discovery 2): Compute G=DxP/I from OpenBCI during THC vs. baseline. If G lands in [0.2123, 0.5000], the Golden Zone finding is empirically confirmed.

3. **Stimulation intensity scaling** (Discovery 4): If hardware reproducing LSD-like state can produce DMT-like state at 1.5x intensity with no parameter changes, the scaling relationship is validated.
...
5. **State transition through MDMA centroid** (Discovery 9): Test if transitioning between states via MDMA-like intermediate requires less total parameter change than direct transitions.
