...
phase_4_nexus6_scan() {
    ...
    if test -f "$n6_script"; then
        ...
        write_bus "nexus6_scan" "ok" "completed"
    else
        local alt_n6
        for alt_n6 in \
            "$BW_ROOT/tools/nexus6/scripts/n6.py" \
            "$HOME/Dev/n6-architecture/tools/nexus6/scripts/n6.py"; do
            if test -f "$alt_n6"; then
                python3 "$alt_n6" scan > /dev/null 2>&1 || true
                log_info "  NEXUS-6 scan complete (alt)"
                write_bus "nexus6_scan" "ok" "alt_path"
                return
            fi
        done
        log_warn "  NEXUS-6 n6.py not found"
        write_bus "nexus6_scan" "skip" "not found"
    fi
}
...
phase_8_nexus6_sync() {
    ...
    if test -f "$sync_script"; then
        ...
        write_bus "nexus6_sync" "ok" "synced"
    else
        if test -f "$BW_ROOT/.shared/sync-nexus6-lenses.sh"; then
            bash "$BW_ROOT/.shared/sync-nexus6-lenses.sh" > /dev/null 2>&1 || true
            log_info "  Lens sync complete"
            write_bus "nexus6_sync" "ok" "lens_sync"
        else
            log_warn "  No sync script found"
            write_bus "nexus6_sync" "skip" "no sync script"
        fi
    fi
}
